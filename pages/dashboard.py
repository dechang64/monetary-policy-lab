...
def render():
    ...
    summary = engine.cumulative_by_asset(event_window_pre=1, event_window_post=5)
    ...
    with col_left:
        ...
        policy_responses = np.random.uniform(-0.25, 0.10, len(assets)).round(3)
        info_responses = np.random.uniform(-0.10, 0.20, len(assets)).round(3)
