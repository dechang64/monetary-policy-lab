...
class FREDConnector:
    ...
    SERIES_MAP = {
        ...
        # Fed Funds Futures (for Kuttner surprise calculation)
        "FF1M Futures": ("FF", "d"),
        "FF3M Futures": ("FFC3M", "d"),
    }
    ...
    def get_fomc_event_windows(
        ...
        """
        Extract event windows around FOMC dates from a DataFrame.

        Returns list of dicts: {fomc_date, window_df, pre_df, post_df}
        """
        windows = []
        for date_str in fomc_dates:
            fomc_ts = pd.Timestamp(date_str)
            pre_start = fomc_ts - pd.Timedelta(days=window_pre * 3)
            post_end = fomc_ts + pd.Timedelta(days=window_post * 3)

            mask = (df.index >= pre_start) & (df.index <= post_end)
            window_df = df.loc[mask]

            pre_mask = (df.index >= pre_start) & (df.index < fomc_ts)
            post_mask = (df.index >= fomc_ts) & (df.index <= post_end)
