...
1. **Loughran-McDonald (2011) financial sentiment dictionary**: Standard positive and negative word lists for financial text analysis, comprising 354 positive terms and 2,329 negative terms in the master dictionary (Loughran and McDonald, 2011), of which a subset appears in FOMC text.

2. **Central bank-specific hawkish/dovish terms**: Drawing on Apel and Blix (2014), Henry (2008), Cieslak et al. (2019), and Hansen et al. (2018), we compile 591 hawkish terms and 222 dovish terms. The hawkish set includes terms such as "tighten," "inflationary pressure," "restrictive stance," "vigilant," "overshoot," "preemptive," and compound phrases like "above-target," "upside-risk," "wage-pressure," and "capacity-constraint." The dovish set includes terms such as "accommodative," "patient," "data-dependent," "balanced approach," and compound phrases like "downside-risk," "labor-market-slack," and "below-potential." The expanded dictionary includes both single words and compound phrases (hyphenated and spaced variants) to capture the full range of FOMC language.

3. **Bigram phrases**: Multi-word expressions that carry hawkish or dovish connotations (e.g., "rate hike," "inflation expectations," "labor market slack," "downside risks"). These are included as part of the hawkish/dovish term lists.
...
2. **CRSP equal-weighted return (ewretd)**: The equal-weighted return, which gives more weight to small-cap stocks. The comparison between value-weighted and equal-weighted returns provides a test of heterogeneous effects of monetary policy across firm sizes.

3. **S&P 500 return (sprtrn)**: The S&P 500 total return index, which is the most widely followed equity benchmark.
...
1. **Rate change** (naive): The observed change in the target federal funds rate. R² = 0.17%, p = 0.712. The rate change is a poor predictor of sentiment because it equals zero for the majority of FOMC meetings (those with no rate change), compressing the variance and attenuating the regression coefficient.

2. **GSS target shock** (proper identification): The high-frequency target rate surprise from G\"urkaynak et al. (2005). R² = 1.57%, p = 0.032. The target shock captures the unexpected component of the rate decision, which has genuine variation even for meetings with no rate change (because the no-change decision itself may be a surprise).

3. **GSS target + path shocks** (full decomposition): Both the target and path factors. R² = 4.12%, path p = 0.017. Adding the path shock more than doubles the R² and reveals the path factor as the more significant predictor.

[Table 3 about here]

The improvement from rate change to GSS shocks is substantial: R² increases by a factor of 9 (from 0.17% to 1.57%), and the target shock becomes statistically significant. Adding the path shock further doubles the R² (from 1.57% to 4.12%) and reveals the path factor as the more significant predictor. The total improvement from the naive measure to the full decomposition is a 24-fold increase in R².
...
- **S&P 500**: Target $\\beta$ = $-$0.259, t = $-$2.193, p < 0.05. R² = 2.9%. An unexpected 1-standard-deviation tightening is associated with a 26 basis point decline in S&P 500 returns.

- **NASDAQ**: Target $\\beta$ = $-$0.282, t = $-$2.089, p < 0.05. R² = 3.4%. The slightly larger coefficient for NASDAQ is consistent with the technology sector's greater sensitivity to interest rate changes.

- **Gold**: Target $\\beta$ = $-$0.404, t = $-$2.465, p < 0.05. R² = 7.0%. Gold has the largest response to target shocks among the assets we examine, consistent with its role as a monetary policy hedge.
...
Excluding the COVID period (March–June 2020) has minimal effect on the results: R² = 4.19% with 115 observations, compared to 4.12% with 117. The path shock remains significant at the 5% level (p = 0.016). This suggests that the COVID meetings are not driving the results, which is important because the COVID period featured unprecedented policy actions and market volatility that could potentially distort the estimates.
...
| Sentiment Measure | R² | β_path | p-value (path) | N |
|-------------------|:--:|:------:|:--------------:|:--:|
| Combined (LM + CB) | 4.12% | 0.000605 | 0.017 | 117 |
| LM only | 1.23% | 0.000198 | 0.421 | 117 |
| CB only | 6.84% | 0.001841 | 0.003 | 117 |

The CB dictionary substantially outperforms the LM dictionary, both in terms of R² (6.84% vs. 1.23%) and statistical significance (p = 0.003 vs. p = 0.421). This is consistent with the well-known positivity bias of the LM dictionary in the context of central bank communication: FOMC statements use more positive than negative words regardless of policy stance, making the LM score a noisy measure of hawkish-dovish content. The CB dictionary, which was specifically designed for central bank communication, captures the relevant semantic variation more effectively.

The combined score (R² = 4.12%) performs worse than the CB score alone (R² = 6.84%), suggesting that the equal-weighted combination dilutes the CB signal with the noisy LM component. A weighted combination that places more weight on the CB component would likely improve performance, but we use equal weighting for transparency and to avoid data-snooping concerns. This finding highlights an important limitation of our approach and suggests that future work should explore more sophisticated methods for combining sentiment measures.
...
3. The information channel interpretation is consistent with the Bauer-Swanson critique: if the path shock reflects the revelation of the Fed's private information about economic conditions, then it is precisely this information revelation that drives language, not an exogenous policy shock. In this interpretation, the Bauer-Swanson finding that surprises are partially predictable does not invalidate the information channel — it simply reflects the fact that the Fed's information is partially shared with the market before the FOMC announcement.

4. We note that the Bauer-Swanson critique applies equally to the target shock and the path shock. If predictable information contaminates both shocks, then the relative importance of the path shock for language is still informative, because it shows that the forward guidance dimension of the surprise is more relevant for communication than the rate decision dimension.
...
| Chair | N | R² | β_target (p) | β_path (p) |
|-------|:--:|:--:|:------------:|:----------:|
| Bernanke | 67 | 5.2% | 0.000312 (0.182) | 0.000724 (0.038) |
| Powell | 50 | 3.8% | 0.000189 (0.452) | 0.000498 (0.124) |

The path shock is significant during the Bernanke era (p = 0.038) but not during the Powell era (p = 0.124), possibly reflecting the smaller sample size and the unusual dynamics of the COVID period. The target shock is not significant in either subsample. The R² is somewhat higher during the Bernanke era, consistent with the more active use of forward guidance during the ZLB period.
...
The modest R² (4.12%) suggests that monetary policy shocks explain only a small fraction of the variation in FOMC language. This is not surprising: FOMC statements are complex documents that convey information about a wide range of economic conditions, policy considerations, and risk assessments, only a subset of which is captured by the target and path shocks. The remaining variation likely reflects the Fed's response to incoming economic data, changes in the Committee's risk assessment, and institutional factors such as the rotation of voting members.

### 6.12 Identification Discussion
...
Second, the modest R² suggests that FOMC statements convey substantial information beyond what is captured by monetary policy shocks. This implies that the Fed's communication strategy is not merely a mechanical response to the rate decision and forward guidance, but reflects a broader assessment of economic conditions and risks. Policymakers should be aware that market participants extract information from the nuances of FOMC language, not just from the rate decision itself.
...
### 6.15 Extended Sample with USMPD
...
**Data Availability.** The monetary policy shock data from Acosta (2022) and the USMPD data from the Federal Reserve Bank of San Francisco are publicly available. FOMC statements are available from the Federal Reserve website. CRSP market data are available through WRDS with an institutional subscription. The replication code and processed datasets will be made available upon publication.
...
Apel, M., & Blix, G. (2014). How Is Inflation Affected by Globalisation? The Riksbank's View. *Sveriges Riksbank Economic Review*, 2014(1), 67–86.
...
G\"urkaynak, R. S., Sack, B. P., & Swanson, E. T. (2005a). The Sensitivity of Long-Term Interest Rates to Economic News: Evidence and Implications for Monetary Policy. *American Economic Review*, 95(1), 425–436.
...
Huang, A. H., Wang, H., & Yang, Y. (2022). FinBERT—A Large Language Model for Extracting Information from Financial Text. *Contemporary Accounting Research*, 39(2), 1019–1046.
...
| FOMC statements | Federal Reserve website | Per FOMC meeting | 2006–2026 |
| CRSP VW/EW/S&P 500 returns | WRDS (crsp.dsi) | Daily | 1990–2024 |
...
| Effective Fed Funds Rate | FRED (DFF) | Daily | 1954–2025 |
| Financial sector stocks | WRDS (crsp.dsf) | Daily | 2020–2025 |
...
### C.2 Extended Sample (Acosta 2006–2022 + USMPD 2022–2026)
