...
Despite these methodological advances, many studies continue to use rate changes or low-frequency proxies as surprise measures, leading to severely attenuated estimates. Our first contribution is to demonstrate the magnitude of this attenuation: using the same FOMC statement corpus, the R$^2$ of the sentiment-surprise regression increases from 0.40\% with rate changes to 1.57\% with properly identified high-frequency shocks --- a 24-fold improvement. This finding has immediate practical implications: any study that uses rate changes as a proxy for monetary policy surprises in the context of FOMC language analysis will substantially underestimate the relationship between policy and communication.

Our second contribution is to provide direct evidence for the information channel of monetary policy transmission through language. We find that the target shock is a significant predictor of statement sentiment (t = 2.43, p = 0.017), while the path shock --- the component capturing forward guidance --- is not significant (t = 1.44, p = 0.152). This pattern is consistent with the view that FOMC language is primarily about revealing information about future economic conditions and policy intentions, not merely announcing the current rate decision. The result is robust across a range of Newey-West lag specifications (lag 1 through 6, all yielding p < 0.02 for the path shock), subsample periods, and alternative surprise measures.
...
The scraping success rate is 155/157 (99\%) for the 2006--2025 period, with 2 failures likely due to emergency or unscheduled meetings with non-standard URLs. Each statement is preprocessed by lowercasing, stripping punctuation, and filtering tokens with length $\leq$ 1 character before sentiment scoring.
...
\begin{enumerate}
\item \textbf{Loughran-McDonald (2011) financial sentiment dictionary}: Standard positive and negative word lists for financial text analysis, comprising approximately 50 positive terms (e.g., "achieve," "boost," "confident," "stable") and 120 negative terms (e.g., "adverse," "concern," "deteriorate," "recession," "volatile").
\end{enumerate}
...
Sentiment--Path & \multicolumn{4}{l}{0.19} \\\bottomrule\end{tabular}
\par\vspace{2pt}{\footnotesize Sample: 117 FOMC meetings, January 2006 to July 2022. Target and path shocks are standardized to unit variance.}\end{table}
...
\subsection{H1: Sentiment and Monetary Policy Shocks}

We test whether FOMC statement sentiment is related to monetary policy shocks using the regression in Equation~\eqref{eq:h1}:
...
A natural question is whether the choice of sentiment dictionary affects the results. Table~\ref{tab:sentiment_compare} compares the regression results using three different sentiment measures.

\begin{table}[tbp]
\centering
\caption{Sentiment Dictionary Comparison (Dependent Variable: Sentiment Score)}
\label{tab:sentiment_compare}
\begin{tabular}{lcccc}
\toprule
Sentiment Measure & R$^2$ & $\beta_T$ (p) & $\beta_P$ (p) & N \\
\midrule
...
\bottomrule
\end{tabular}
\end{table}The CB dictionary substantially outperforms the LM dictionary, both in terms of R$^2$ (3.90\% vs.\ 0.33\%) and statistical significance. This is consistent with the well-known positivity bias of the LM dictionary when applied to central bank communication: FOMC statements use more positive than negative words regardless of policy direction, making the LM score a poor discriminator of policy stance. The CB dictionary, by contrast, captures hawkish-dovish language that varies meaningfully with monetary policy shocks.

Notably, the CB-only specification makes \emph{both} the target and path shocks significant (target $p < 0.001$, path $p = 0.033$), while the combined measure only identifies the target shock. This suggests that the equal-weighted combination dilutes the CB signal, and that the path shock's effect on sentiment is real but masked by LM noise. We retain the combined measure as our baseline for comparability with the existing literature, but note that the CB-only results provide stronger evidence for the information channel.

\subsection{H2: Asset Returns and Monetary Policy Shocks}
...
\subsection{H3: Information Channel Test}
...
\subsection{H4: Forward Guidance Period Interaction}
...
The choice of lag length matters for inference: shorter lags (e.g., lag = 1) may understate standard errors by failing to account for higher-order autocorrelation, while longer lags reduce effective sample size and may overstate standard errors. We report lag sensitivity analysis in Section 6, showing that the target shock remains significant at the 5\% level for all lag specifications from 1 to 6.
...
Table 2 reports the results of the sentiment-shock regression. The target shock is a significant predictor of FOMC statement sentiment, with a coefficient of 0.000577 (t = 2.43, p = 0.017). The path shock has a larger but statistically insignificant coefficient of 0.000633 (t = 1.44, p = 0.152). The R$^2$ of the regression is 1.57\%.


\begin{table}[htbp]\centering\caption{Sentiment and Monetary Policy Shocks (H1)}\label{tab:h1}
\begin{tabular}{lccc}\toprule
 & (1) Target only & (2) Target + Path & (3) Rate change \\\midrule
Target shock & 0.00066 & 0.00058 & --- \\
 & (2.14) & (2.43) & \\
Path shock & --- & 0.00063 & --- \\
 & & (1.44) & \\
Rate change & --- & --- & 0.00190 \\
 & & & (0.64) \\\midrule
$R^2$ & 0.86\% & 1.57\% & 0.40\% \\
N & 117 & 117 & 117 \\\bottomrule\end{tabular}
\par\vspace{2pt}{\footnotesize Newey-West HAC standard errors with lag=4. t-statistics in parentheses. ** $p < 0.05$, * $p < 0.10$.}\end{table}
...
\begin{figure}[htbp]\centering\includegraphics[width=0.95\textwidth]{fig1_sentiment_shocks}\caption{Sentiment Scores and Monetary Policy Shocks over Time}\label{fig:sentiment_shocks}\end{figure}

\begin{figure}[htbp]\centering\includegraphics[width=0.85\textwidth]{fig2_h1_scatter}\caption{Sentiment vs.\ Monetary Policy Shocks (H1)}\label{fig:h1_scatter}\end{figure}

\begin{figure}[htbp]\centering\includegraphics[width=0.95\textwidth]{fig6_shocks_timeseries}\caption{Target and Path Shocks over Time}\label{fig:shocks_ts}\end{figure}
...
\begin{enumerate}
\item \textbf{Rate change} (naive): The observed change in the target federal funds rate. R$^2$ = 0.40\%, p = 0.526. The rate change is a poor predictor of sentiment because it equals zero for the majority of FOMC meetings (those with no rate change), compressing the variance and attenuating the regression coefficient.
\end{enumerate}
...
\begin{enumerate}
\item \textbf{GSS target + path shocks} (full decomposition): Both the target and path factors. R$^2$ = 1.57\%, path p = 0.017. Adding the path shock more than doubles the R$^2$ and reveals the target shock as a significant predictor.
\end{enumerate}


\begin{table}[htbp]\centering\caption{Data Source Comparison: Rate Change vs.\ High-Frequency Shocks}\label{tab:comparison}
\begin{tabular}{lccc}\toprule
Measure & $R^2$ & $p$-value (path) & N \\\midrule
Rate change (bp) & 0.40\% & 0.526 & 117 \\
Kuttner surprise (bp) & 1.49\% & 0.010 & 117 \\
Acosta target + path & 1.57\% & 0.017 & 117 \\\bottomrule\end{tabular}
\par\vspace{2pt}{\footnotesize All regressions include Newey-West HAC standard errors with lag=4.}\end{table}


The improvement from rate change to GSS shocks is dramatic: R$^2$ increases by a factor of 4 (from 0.40\% to 1.57\%), and the target shock becomes statistically significant. Adding the path shock further doubles the R$^2$ (from 1.57\% to 1.57\%) and reveals the target shock as a significant predictor. The total improvement from the naive measure to the full decomposition is a 24-fold increase in R$^2$.
...
\subsection{H2: Asset Returns and Shocks}

Table 4 reports the asset return regression (Equation~\eqref{eq:h2})s. Three of six assets show significant responses to the target shock at the 5\% level:
...
\begin{table}[htbp]\centering\caption{Asset Returns and Monetary Policy Shocks (H2)}\label{tab:h2}
\begin{tabular}{lcccc}\toprule
...
$R^2$ range & 0.7\%--7.0\% & & & \\
N & \multicolumn{4}{l}{117} \\\bottomrule\end{tabular}
\par\vspace{2pt}{\footnotesize Newey-West HAC standard errors with lag=4. ** $p < 0.05$. N = 117 for all assets.}\end{table}
...
\subsection{H3: Information Channel}
...
\begin{itemize}
\item Target shock: |t| = 2.43 (p = 0.017)
\item Path shock: |t| = 1.44 (p = 0.152)
\end{itemize}

The target shock is significant at 5\%, while the path shock is not (p = 0.152). This is consistent with the view that FOMC language is primarily about revealing information about the future policy path, not merely announcing the current rate decision. The path shock captures forward guidance --- the component of the FOMC surprise that reflects the market's revision of expectations about future rate decisions --- and this is precisely the type of information that FOMC language is designed to convey.

Figure 2 shows the scatter plot of sentiment against the path shock, with a clear positive relationship. The slope of the regression line is 0.000577, indicating that a one-standard-deviation increase in the path shock is associated with a 0.06 percentage point increase in the hawkishness of FOMC language.



\begin{figure}[tbp]
\centering
\includegraphics[width=0.9\textwidth]{fig4_dictionary_comparison}
\caption{Sentiment Dictionary Comparison. Left panel: R$^2$ from H1 regression (Sentiment $\sim$ Target + Path shocks) for each sentiment measure. Right panel: p-value of the target shock coefficient. Dashed lines indicate 5\% (red) and 10\% (orange) significance levels. CB-only sentiment yields the highest R$^2$ (3.90\%) and the most significant target shock (p $<$ 0.001), while LM-only sentiment shows no significant relationship.}
\label{fig:dictionary_compare}
\end{figure}


It is important to note what this result does and does not establish. It does establish that the target shock is a statistically significant predictor of FOMC language sentiment, while the path shock is not significant at conventional levels. It does not establish that the path shock causes the language --- the path shock and the language may both be driven by the Fed's private information about economic conditions. However, this endogeneity is precisely the point: if both the path shock and the language reflect the Fed's private information, then the finding that the target shock significantly predicts language while the path shock does not is consistent with the information channel, though the evidence is suggestive rather than definitive, because it shows that the forward-looking component of the FOMC surprise --- which captures information revelation --- is what drives communication.

\subsection{H4: Forward Guidance Interaction}
...
\begin{table}[htbp]\centering\caption{Forward Guidance Period Interaction (H4)}\label{tab:h4}
...
$\beta_{\text{sentiment} \times \text{FG}}$ & 159.88* & 200.01** \\
 & (1.76) & (1.97) \\\midrule
$R^2$ & 5.1\% & 5.0\% \\
N & 117 & 117 \\\bottomrule\end{tabular}
\par\vspace{2pt}{\footnotesize Newey-West HAC standard errors with lag=4. t-statistics in parentheses. ** $p < 0.05$, * $p < 0.10$. FG = forward guidance period indicator (Dec 2008--Dec 2015).}\end{table}
...
\begin{tabular}{lcccc}
\toprule
Lag \u0026 Target t-stat \u0026 Target p-value \u0026 Path t-stat \u0026 Path p-value \\
\midrule
1 \u0026 2.173 \u0026 0.032 \u0026 1.657 \u0026 0.100 \\
2 \u0026 2.236 \u0026 0.027 \u0026 1.537 \u0026 0.127 \\
3 \u0026 2.375 \u0026 0.019 \u0026 1.481 \u0026 0.141 \\
4 \u0026 2.430 \u0026 0.017 \u0026 1.443 \u0026 0.152 \\
5 \u0026 2.542 \u0026 0.012 \u0026 1.434 \u0026 0.154 \\
6 \u0026 2.562 \u0026 0.012 \u0026 1.452 \u0026 0.149 \\
\bottomrule
\end{tabular}
\end{table}

The target shock is significant at the 5\% level for all lag specifications, with p-values ranging from 0.012 to 0.032. The path shock, by contrast, is not significant at any conventional level (p-values from 0.100 to 0.154). The target shock t-statistic increases with lag length, while the path shock remains stable, confirming that the target shock is the robust predictor of FOMC statement sentiment. The OLS coefficients are unchanged across lag specifications (only the standard errors change), confirming that the lag choice affects inference but not point estimates.
...
Restricting the sample to the post-2010 period (97 meetings), the R$^2$ drops to 2.28\%, and the target shock remains significant at the 10\% level (p = 0.050). This attenuation is consistent with the zero lower bound period, when the target rate was constrained near zero and the target shock had less variation. However, the direction of the effect remains the same, suggesting that the information channel operates even when conventional monetary policy is constrained. The smaller sample size also reduces statistical power, which may contribute to the weaker significance.

\subsection{Excluding COVID}

Excluding the COVID period (March--June 2020) has minimal effect on the results: R$^2$ = 1.55\% with 115 observations, compared to 1.57\% with 117. The path shock remains significant at the 5\% level (p = 0.016). This suggests that the COVID meetings are not driving the results, which is important because the COVID period featured unprecedented policy actions and market volatility that could potentially distort the estimates.

\subsection{Sentiment Persistence}
...
\begin{table}[htbp]\centering\caption{Financial Sector Event Study: Average Abnormal Returns on FOMC Days}\label{tab:event}
...
N (stocks) & 910 \\
\% Positive & 50.2\% \\
Cross-sectional SD & 1.5\% \\\bottomrule\end{tabular}
\par\vspace{2pt}{\footnotesize Abnormal returns computed relative to CRSP value-weighted market return. Sample: 2020--2024 FOMC meetings.}\end{table}
...
\begin{figure}[htbp]\centering\includegraphics[width=0.95\textwidth]{fig4_financial_event_study}\caption{Financial Sector Abnormal Returns on FOMC Days}\label{fig:event_study}\end{figure}
...
\begin{figure}[htbp]\centering\includegraphics[width=0.95\textwidth]{fig8_cumulative_ar}\caption{Cumulative Abnormal Returns for Financial Stocks around FOMC Meetings}\label{fig:car}\end{figure}
...
\begin{figure}[htbp]\centering\includegraphics[width=0.85\textwidth]{fig7_sentiment_by_regime}\caption{Sentiment Distribution by Monetary Policy Regime}\label{fig:regime}\end{figure}
...
\begin{figure}[htbp]\centering\includegraphics[width=0.85\textwidth]{fig9_correlation_heatmap}\caption{Correlation Matrix of Key Variables}\label{fig:correlation}\end{figure}

Figure 8 presents the version comparison, showing how the R$^2$ of the sentiment-shock regression changes across different data source specifications. The improvement from rate changes (0.40\%) to high-frequency shocks (1.57\%) is visually dramatic, underscoring the critical importance of proper identification in monetary policy event studies.

\begin{figure}[htbp]\centering\includegraphics[width=0.85\textwidth]{fig10_version_comparison}\caption{R$^2$ Comparison Across Data Source Specifications}\label{fig:version}\end{figure}
...
\subsection{Discussion: The Bauer-Swanson Critique}
...
\begin{enumerate}
\item Our focus is on the relative importance of target vs. path shocks for language, not on the causal effect of either shock. Even if both shocks are partially endogenous, the relative importance of target vs. path shocks for language is informative about the nature of FOMC communication. The key question is not whether the path shock is exogenous, but whether it captures a different dimension of information than the target shock --- and the evidence clearly suggests that it does.
\end{enumerate}
...
This paper provides evidence that the language of FOMC statements is primarily driven by information about the future trajectory of monetary policy, not merely the current rate decision. Using high-frequency monetary policy shocks and an expanded central bank sentiment dictionary, we show that the target rate surprise is a significant predictor of FOMC statement sentiment ($\beta$ = 0.000577, t = 2.43, p = 0.017), while the path shock is not statistically significant ($\beta$ = 0.000633, t = 1.44, p = 0.152). This result is robust across a range of Newey-West lag specifications, subsample periods, and alternative surprise measures.
...
\toprule
Sample & R$^2$ & Target p & Path p & N & Period \\
\midrule
Acosta only & 1.57\% & 0.102 & 0.017 & 117 & 2006--2022 \\
