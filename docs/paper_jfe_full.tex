...
\bottomrule
\end{tabular}
\footnotesize{Sample: 117 FOMC meetings, January 2006 to July 2022. Target and path shocks are standardized to unit variance. Kuttner surprise in basis points.}
\end{table}
...
\bottomrule
\end{tabular}
\footnotesize{Newey-West HAC standard errors with lag=4. t-statistics in parentheses. ** $p $<$ 0.05$, * $p $<$ 0.10$.}
\end{table}
...
\bottomrule
\end{tabular}
\footnotesize{All regressions include Newey-West HAC standard errors with lag=4.}
\end{table}
...
\begin{tabular}{lcccc}
\toprule
Asset & $\beta_{target}$ & $t$ & $\beta_{path}$ & $t$ \\
\midrule
...
\bottomrule
\end{tabular}
\footnotesize{Newey-West HAC standard errors with lag=4. ** $p $<$ 0.05$. N = 117 for all assets.}
\end{table}
...
\toprule
 & S\&P 500 & NASDAQ \\
\midrule
$\beta_{target}$ & $-$0.287** & $-$0.306** \\
 & ($-$2.59) & ($-$2.45) \\
$\beta_{sentiment}$ & 41.71 & 0.99 \\
 & (0.99) & (0.02) \\
$\beta_{sentiment \times FG}$ & 159.88* & 200.01** \\
 & (1.76) & (1.97) \\
\midrule
$R^2$ & 5.1\% & 5.0\% \\
N & 117 & 117 \\
\bottomrule
\end{tabular}
\footnotesize{Newey-West HAC standard errors with lag=4. t-statistics in parentheses. ** $p $<$ 0.05$, * $p $<$ 0.10$. FG = forward guidance period indicator (Dec 2008--Dec 2015).}
\end{table}
...
\bottomrule
\end{tabular}
\footnotesize{Abnormal returns computed relative to CRSP value-weighted market return. Sample: 2020--2024 FOMC meetings.}
\end{table}
...
Gürkaynak, R. S., Sack, B. P., & Swanson, E. T. (2005a). The Sensitivity of Long-Term Interest Rates to Economic News: Evidence and Implications for Monetary Policy. \textit{American Economic Review}, 95(1), 425–436.
...
Jarociński, M., & Karadi, P. (2020). Deconstructing Monetary Policy Surprises---The Role of Information Shocks. \textit{American Economic Journal: Macroeconomics}, 12(2), 1–43.
...
The LM score is always positive for FOMC statements (min = 0.006), because FOMC statements use more positive than negative words regardless of policy stance. The CB component has substantial sign variation (78\% negative), but the equal-weighted combination dilutes this signal.\\end{document}
