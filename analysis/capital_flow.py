    ...
    def risk_regime_analysis(
        ...
        for fomc_date in self.fomc_dates:
            ...
            for d in range(-window, 0):
                ...
                if target in self.returns.index:
                    risk_assets = [
                        a for a in self.RISK_CATEGORIES["Risk-On"]
                        if a in self.returns.columns
                    ]
                    safe_assets = [
                        a for a in self.RISK_CATEGORIES["Risk-Off"]
                        if a in self.returns.columns
                    ]
    ...
    def correlation_change(
        ...
        for fomc_date in self.fomc_dates:
            ...
            if len(pre_data) > 10 and len(post_data) > 10:
                ...
                pre_avg = (pre_corr.values.sum() - pre_corr.shape[0]) / (
                    pre_corr.shape[0] * (pre_corr.shape[0] - 1)
                )
                post_avg = (post_corr.values.sum() - post_corr.shape[0]) / (
                    post_corr.shape[0] * (post_corr.shape[0] - 1)
                )
